#Brady McGonagle
#Speech Feedback System - desktop app
...
class SpeechApp(tk.Tk):
    ...
    def _build(self):
        ...
        #Read-only: editing here would look like it saved, and it wouldn't.
    ...
    def _reconcile(self):
        """
        Re-score every stored transcript with the current FILLERS/MARKERS.

        Runs silently at startup. Without it, sessions counted under an older
        word list sit alongside new ones and the trend compares definitions
        rather than speaking. Cheap: regex over text already in the database.
        """
        try:
    ...
    def _lock(self, busy):
        """Disable the actions that can't safely overlap a running job."""
        self.busy = busy
    ...
    def _process(self, audio, source, source_path, label=None):
        """
        Shared tail of every ingest path: save, transcribe, count, log.
        Runs on a worker thread. Returns nothing; reports through self.jobs.
        """
    ...
    def _record_worker(self):
        ...
        This mirrors record() in speech_test_V1 but can't reuse it: that one
        blocks until silence, prints progress to stdout, and offers no way to
        cancel. Here the stop button and the silence timer both end the loop.
        """
        try:
    ...
    def _draw_meter(self, db):
        """Map roughly -60..0 dB onto a 120px bar."""
    ...
    def _file_worker(self, paths):
        """Ingest one or many files. Skips failures rather than aborting the batch."""
        try:
            import audio_io
            done, failed = 0, []
