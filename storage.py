...
Design note: this stores the raw transcript and duration alongside the derived
counts. The counts make stats queries trivial; the transcript means you can
recompute every past session when you change FILLERS or MARKERS. Derived
numbers alone would silently rot as your word lists evolve.
"""
...
def summary(conn, engine=None):
    ...
    Segmented by engine on purpose. Google strips filled pauses and Whisper
    keeps them, so pooling both makes your filler rate drop the day you switch
    engines and look like progress. It isn't.
    """
    q = """SELECT engine,
                  COUNT(*)                      AS sessions,
...
def trend(conn, engine=None, window=5):
    """
    Rolling wpm and fillers/min over the last `window` sessions vs the previous
    `window`. Returns None if there isn't enough history to compare.
    """
